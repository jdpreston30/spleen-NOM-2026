...
#+ 5.1: Cohort, outcome, and candidate predictors
# The analysis cohort is already NOM-only (operative-index patients excluded in 0d.1.2);
# here we additionally drop early (<24 h) deaths, who have no success/failure outcome.
...
#+ 5.2: Univariable candidate screen (Table 6) — each unadjusted, complete-case per predictor
...
uni_rows <- lapply(names(candidates), function(lbl) {
  ...
  tibble(
    ...
    # Column must be named "P" to match the ternG tables and to trigger bold_sig
...
#+ 5.3: Multivariable model (Table 7) — pre-specified, complete predictors only
...
# Transfusion axis is represented by MTP activation rather than a "received any blood"
# indicator: MTP specifically marks massive hemorrhage, whereas any-transfusion is a low
# bar (53% of the cohort). MTP was only analyzable after its two era-specific source
# columns were reconciled during cleaning; both are reported univariably in Table 5.
nom_fit <- glm(nom_failure ~ grade_V + map10 + mtp, data = nom_data, family = binomial)
